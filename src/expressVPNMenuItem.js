...
const ExpressVPNMenuItem = new Lang.Class({
    Name: 'ExpressVPNMenu.ExpressVPNMenuItem',
    Extends: PopupMenu.PopupMenuItem,

    _init : function(code, country) {
      	this.parent(country);
    ...
    _clickAction : function() {
        ...
        let success, pid, in_fd, out_fd, err_fd;

        log('Connecting to: ' + currentCountry);
        Util.showNotification(_('ExpressVPN Connecting'), _('Connecting to ') + currentCountry + '...');

        [success, pid, in_fd, out_fd, err_fd] = GLib.spawn_async_with_pipes(
            null,
            ['/bin/bash', '-c', 'expressvpn connect ' + this.code],
            null,
        ...
        if (success  &&  pid !== 0) {
            ...
            GLib.child_watch_add( GLib.PRIORITY_DEFAULT, pid, function(pid,status) {
                ...
                Util.expressVPNStatusNotification('connect');
            });
        } else {
